...
for(i in 1:length(Mz)){
    ...
    mz<-Mz[i]
    out.Sigma<-gen.spikedcov(n,m.0,tau,beta)
    sigma.Y<- sqrt(diag(A%*%out.Sigma$Sigma.Y%*%t(A)))
    
    Sigma.true<-out.Sigma$Sigma
    b.tilde<- b/(b+h)
    ...
    result<-foreach(N = 1:reps,.export=c("mvrnorm","EsaBcv","POET","Factmle_cov"))%dopar%{
      ...
      set.seed(N)
      W<- mvrnorm(mz,rep(0,n),Sigma.true)
      S<- cov(W)*(mz/(mz-1))
      
      K.est<- KN.test(S, mz, 0.0005)
      K.est$numOfSpikes<- max(1,K.est$numOfSpikes)
      ...
      nfactors<-c(K.est$numOfSpikes,K.Bcv,K.poet,K.est$numOfSpikes)
      ...
      out.q<- q.est.agg(out.rmt,A,X,b.tilde,taubeta.q[1],
                    taubeta.q[2],eta,m,m.0,0)
      qhat<-out.q$q
      f<-out.q$f
      
      qhat.1<- q.est.agg(out.rmt,A,X,b.tilde,taubeta.q[1],
                     taubeta.q[2],eta,m,m.0,1)$q
      qhat.Bayes<- q.Bayes.agg(Sigma.true,A,X,b.tilde,tau,beta,eta,m,m.0)
      qhat.naive<- q.naive.1.agg(S.naive,A,X,b.tilde,
                             taubeta.naive[1],taubeta.naive[2],eta,m,m.0)
      qhat.Bcv<- q.Bcv.agg(S.Bcv,A,X,b.tilde,
                       taubeta.Bcv[1],taubeta.Bcv[2],eta,m,m.0)
      qhat.poet<- q.poet.agg(S.poet,A,X,b.tilde,
                         taubeta.poet[1],taubeta.poet[2],eta,m,m.0)
      qhat.fact<- q.factmle.agg(S.fact,A,X,b.tilde,
                            taubeta.fact[1],taubeta.fact[2],eta,m,m.0)
      
      risk.q<- risk.agg.est(qhat,A%*%theta,sigma.Y,b,h,1)
      risk.1<- risk.agg.est(qhat.1,A%*%theta,sigma.Y,b,h,1)
      risk.Bayes<- risk.agg.est(qhat.Bayes,A%*%theta,sigma.Y,b,h,1)
      risk.naive<- risk.agg.est(qhat.naive,A%*%theta,sigma.Y,b,h,1)
      risk.Bcv<- risk.agg.est(qhat.Bcv,A%*%theta,sigma.Y,b,h,1)
      risk.poet<- risk.agg.est(qhat.poet,A%*%theta,sigma.Y,b,h,1)
      risk.fact<- risk.agg.est(qhat.fact,A%*%theta,sigma.Y,b,h,1)
...
save.image(paste(getwd(),'/final simulations in paper/exp32_linexloss.RData',sep=''))
