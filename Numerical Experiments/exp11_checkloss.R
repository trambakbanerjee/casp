...
l0<-1
set.seed(10)
b<- runif(n,0.9,0.95)#)#check loss parameters
h<- 1-b#rep(0.05,n)#check loss parameters
m<-1 # sample sizes
Mz<- seq(15,50,5)
...
for(i in 1:length(Mz)){
  ...
  result<-foreach(N = 1:reps,.export=c("mvrnorm","EsaBcv","POET","Factmle_cov"))%dopar%{
    ...
    out.q<- q.est(out.rmt,X,b.tilde,taubeta.q[1],
                  taubeta.q[2],eta,m,m.0,0)
    qhat<-out.q$q
    f<-out.q$f
    
    qhat.1<- q.est(out.rmt,X,b.tilde,taubeta.q[1],
                   taubeta.q[2],eta,m,m.0,1)$q
    qhat.Bayes<- q.Bayes(out.Sigma$Sigma,X,b.tilde,tau,beta,eta,m,m.0)
    qhat.naive<- q.naive.1(S.naive,X,b.tilde,
                           taubeta.naive[1],taubeta.naive[2],eta,m,m.0)
    qhat.Bcv<- q.Bcv(S.Bcv,X,b.tilde,
                     taubeta.Bcv[1],taubeta.Bcv[2],eta,m,m.0)
    qhat.poet<- q.poet(S.poet,X,b.tilde,
                       taubeta.poet[1],taubeta.poet[2],eta,m,m.0)
    qhat.fact<- q.factmle(S.fact,X,b.tilde,
                          taubeta.fact[1],taubeta.fact[2],eta,m,m.0)
    
    risk.q<- risk.est(qhat,theta,sigma.Y,b,h,1)
    risk.1<- risk.est(qhat.1,theta,sigma.Y,b,h,1)
    risk.Bayes<- risk.est(qhat.Bayes,theta,sigma.Y,b,h,1)
    risk.naive<- risk.est(qhat.naive,theta,sigma.Y,b,h,1)
    risk.Bcv<- risk.est(qhat.Bcv,theta,sigma.Y,b,h,1)
    risk.poet<- risk.est(qhat.poet,theta,sigma.Y,b,h,1)
    risk.fact<- risk.est(qhat.fact,theta,sigma.Y,b,h,1)
  ...
  stopCluster(cl)
  registerDoSEQ()
  temprisk.q<- sapply(1:reps,function(j) result[[j]]$q)
  temprisk.1<- sapply(1:reps,function(j) result[[j]]$q.1)
  temprisk.Bayes<- sapply(1:reps,function(j) result[[j]]$bayes)
  temprisk.naive<- sapply(1:reps,function(j) result[[j]]$naive)
  temprisk.Bcv<- sapply(1:reps,function(j) result[[j]]$bcv)
  temprisk.poet<- sapply(1:reps,function(j) result[[j]]$poet)
  temprisk.fact<- sapply(1:reps,function(j) result[[j]]$fact)
  taubeta.estimated[i,,]<- rbind(
    rowMeans(sapply(1:reps,function(j) result[[j]]$taubeta[1,])),
    rowMeans(sapply(1:reps,function(j) result[[j]]$taubeta[2,])),
    rowMeans(sapply(1:reps,function(j) result[[j]]$taubeta[3,])),
    rowMeans(sapply(1:reps,function(j) result[[j]]$taubeta[4,])),
    rowMeans(sapply(1:reps,function(j) result[[j]]$taubeta[5,])))
...
save.image(paste(getwd(),'/final simulations in paper/exp11_checkloss_newnew.RData',sep=''))
...
g2<- ggplot()+geom_point(data=shrink,aes(x=dim,y=f),color="red")+
  ...
  #scale_x_continuous(breaks = seq(1,n,length.out = 20))+
